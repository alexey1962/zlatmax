...
function documentActions(e) {
  ...
  if (targetElement.closest('[data-parent]')) {
      ...
      if(subMenu) {
          ...
          const backBtns = document.querySelectorAll('.sub-menu-catalog__back')

          if(activeLink && activeLink !== targetElement) {
            activeLink.classList.remove('_sub-menu-active')
            document.querySelectorAll('._sub-menu-open').forEach(el => {
...
const stockProduct = document.querySelector('.header-product__instock')
if(stockProduct) {
  if (stockProduct.closest('.stock')) {
    stockProduct.innerHTML = 'В наличии'
  } else {
    stockProduct.innerHTML = 'Нет в наличии'
  }
}